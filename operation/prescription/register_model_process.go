...
func (opp *RegisterModelProcessor) PreProcess(
	...
	if found, _ := state.CheckNotExistsState(prstate.DesignStateKey(fact.Contract()), getStateFunc); found {
		return ctx, mitumbase.NewBaseOperationProcessReasonError(
			common.ErrMPreProcess.
				Wrap(common.ErrMServiceE).Errorf("prescription service for contract account %v",
				fact.Contract(),
			)), nil
	}
...
func (opp *RegisterModelProcessor) Process(
	...
	if err := design.IsValid(nil); err != nil {
		return nil, mitumbase.NewBaseOperationProcessReasonError("invalid prescription design, %q; %w", fact.Contract(), err), nil
	}

	sts = append(sts, state.NewStateMergeValue(
		prstate.DesignStateKey(fact.Contract()),
		prstate.NewDesignStateValue(design),
	))
